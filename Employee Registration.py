...
def mysql_connection():
    if check.check_registration():
        ...
        try:
            Q = "INSERT INTO employee (e_firstname, e_lastname, e_address, e_mobile, e_birthday, e_joindate, e_designation) VALUES (UPPER(%s), UPPER(%s), %s, %s, %s, %s, %s)"
            val = (e1, e2, e3, e4, e6, e7, e8)
            cq.execute(Q, val)
            db.commit()
            Q1 = "INSERT INTO emp_pass (e_password) VALUES (%s)"
            val1 = (e5,)
            cq.execute(Q1, val1)
            db.commit()
        #except:
